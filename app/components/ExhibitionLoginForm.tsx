...
export default function ExhibitionLoginForm({ onClose }: { onClose?: () => void }) {
  ...
  function onSubmit(e: React.FormEvent) {
    ...
    startTransition(() => {
      router.push(`/exhibition?id=${student.studentId}`);
    });
  }

  return (
    <form onSubmit={onSubmit} className="mt-4 text-left">
      <label htmlFor="exhStudentId" className="block text-sm font-medium text-foreground">
        เลขประจำตัวนักเรียน
      </label>
      <input
        id="exhStudentId"
      ...
      <div className="mt-5 flex gap-2">
        ...
        >
          {pending ? "กำลังตรวจสอบ…" : "ไปเลือกฝ่าย"}
          <svg
